...
int EDVDB_LoadZipSchema(const char *path)
{
    ...
    ZipTemplateManager::SetTemplate(tem);
    return 0;
}
...
{
    EDVDB_LoadZipSchema(ZipTemPath);//加载压缩模板
    long len;
    ...
    remove(const_cast<char *>(filepath.c_str()));//删除原文件
    long fp;
    string finalpath=filepath.append("zip");//给压缩文件后缀添加zip，暂定，根据后续要求更改
    int err = EDVDB_Open(const_cast<char *>(finalpath.c_str()),"wb",&fp);
    if (err == 0)
    {
